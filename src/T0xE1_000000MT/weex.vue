...
export default {
  ...
  created() {
    ...
    if (this.isIos){ //兼容苹果手机的04监听上报  （parker）
      this.listenerFun();
    }
    //var self = this;
    //模拟设备数据
    ...
    this.queryStatus();
     ...
     nativeService.getDeviceSN(function (resData) {
          nativeService.setItem("sn",resData);
      }, (error) => {
          nativeService.toast(error);
      });

       nativeService.getCurrentApplianceID(function (resData) {
          nativeService.setItem("applianceId",resData);
      }, (error) => {
          nativeService.toast(error);
      });
      ...
      (resp) => {
        ...
        // nativeService.alert(res.userId);
         nativeService.setItem("userId",res.userId);
      }
      ).catch((error) => {
        nativeService.toast(error)
      })
