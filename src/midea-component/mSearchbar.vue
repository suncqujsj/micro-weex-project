<template>
  <div>
    <div :class="['midea-search-bar','midea-search-bar-'+theme]"
         :style="barStyle"
         v-if="mod==='default'">
      <input @blur="onBlur"
             @focus="onFocus"
             ...
             :style="{ width: needShowCancel ? setWidth : setWidth }"
    ...
    methods: {
      onBlur () {
        ...
        setTimeout(() => {
          ...
          self.$emit('mideaSearchbarInputOnBlur', { value: self.value });
        }, 10);
      },
      autoBlur () {
      ...
      onFocus () {
        ...
        this.$emit('mideaSearchbarInputOnFocus', { value: this.value });
      },
      closeClicked () {
        this.value = '';
        this.showCancel && (this.showCancel = false);
        this.showClose && (this.showClose = false);
        this.$emit('mideaSearchbarCloseClicked', { value: this.value });
        this.$emit('mideaSearchbarInputOnInput', { value: this.value });
      },
      onInput (e) {
        this.value = e.value;
        this.showCancel = true;
      ...
      onSubmit (e) {
        ...
        this.showCancel = true;
        this.detectShowClose();
        this.$emit('mideaSearchbarInputReturned', { value: this.value });
      },
      cancelClicked () {
        this.showCancel && (this.showCancel = false);
        this.showClose && (this.showClose = false);
        this.$emit('mideaSearchbarCancelClicked', { value: this.value });
      },
      detectShowClose () {
        this.showClose = (this.value.length > 0) && this.showCancel;
      },
      depClicked () {
        this.$emit('mideaSearchbarDepChooseClicked', {});
      },
      inputDisabledClicked () {
        this.$emit('mideaSearchbarInputDisabledClicked', {});
      },
      setValue (value) {
        this.value = value;
      }
