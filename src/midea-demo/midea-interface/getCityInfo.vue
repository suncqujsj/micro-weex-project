<template>
    <div class="wrapper">
        <midea-header title="getCityInfo获取城市信息" :isImmersion="isImmersion" @leftImgClick="back" :showRightImg="true" rightImg="../assets/img/smart_ic_reline@3x.png" @rightImgClick="reload"></midea-header>
        ...
        <midea-button text="获取城市信息" @mideaButtonClicked="mideaButtonClicked">
        </midea-button>
        <midea-title-bar title="代码"></midea-title-bar>
        <text class="display-block">
            nativeService.getCityInfo(messageParam).then(
                (resp) => {
                    this.result = resp
                }
            )
...
module.exports = {
    ...
    methods: {
        ...
        mideaButtonClicked() {
            nativeService.getCityInfo(this.messageParam).then(
                (resp) => {
                    this.result = resp
                }
            ).catch((error) => {
