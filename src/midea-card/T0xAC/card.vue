...
export default {
    ...
    methods: {
        ...
        onMideachange4(event) {
            let param = {
                url: "/ccrm2-core/userAddr/getDefaultAddr"
            }
            nativeService.sendCentralCloundRequest('', param).then(() => {
                nativeService.toast("设置成功")
            }).catch(() => {
                this.mideaChecked4 = !this.mideaChecked4
                nativeService.toast("设置失败")
            })
            //modal.toast({ 'message': event.value, 'duration': 2 });
        },
