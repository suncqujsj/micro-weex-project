<template>
    <div class="wrapper">
        <midea-header title="baiduGeocoder获取位置信息(百度)" :isImmersion="isImmersion" @leftImgClick="back" :showRightImg="true" rightImg="../assets/img/smart_ic_reline@3x.png" @rightImgClick="reload"></midea-header>
        ...
        <midea-button text="获取位置信息(百度)" @mideaButtonClicked="mideaButtonClicked">
        </midea-button>
        <midea-title-bar title="代码"></midea-title-bar>
        <text class="display-block">
            nativeService.baiduGeocoder(messageParam).then(
                (resp) => {
                    this.result = resp
                }
            )
...
module.exports = {
    ...
    data() {
        return {
            messageParam: {
                latitude: "40.05",  //纬度 string
                longitude: "113.30"//经度  string
            },
            result: ''
        }
    },
    ...
    methods: {
        ...
        mideaButtonClicked() {
            nativeService.baiduGeocoder(this.messageParam).then(
                (resp) => {
                    this.result = resp
                }
            ).catch((error) => {
