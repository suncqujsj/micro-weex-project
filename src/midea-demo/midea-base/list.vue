<template>
    <div class="wrapper">
        ...
        <list>
            <midea-item v-for="(item,i) in list2" :key="item" :hasTopBorder="i===0?true:false" :title="item.name" :hasArrow="false" :clickActivied="true" @onItemPress="showDel(item)" :canEdit="true" @onDelete="delItem(item)">
                <midea-switch2 :checked="mideaChecked" @change="onMideachange" width="106" height="48" slot="value"></midea-switch2>
            </midea-item>
            <midea-item v-for="(item,i) in list" :key="item" :hasTopBorder="false" :title="item.name" :hasArrow="true" :clickActivied="true" @onItemPress="showDel(item)" :canEdit="false" @onDelete="delItem(item)">
            </midea-item>
            <midea-item v-for="(item,i) in list1" :key="item" :hasTopBorder="false" :title="item.name" :hasArrow="false" :desc="item.desc" :itemImg="item.itemImg" height="220" :clickActivied="true" @onItemPress="showDel(item)" :canEdit="true" @onDelete="delItem(item)">
            </midea-item>
