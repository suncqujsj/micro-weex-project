<template>
    <div class="wrapper">
        <midea-header title="getAddressBookPerson通讯录" :isImmersion="isImmersion" @leftImgClick="back" :showRightImg="true" rightImg="../assets/img/smart_ic_reline@3x.png" @rightImgClick="reload"></midea-header>
        <midea-title-bar title="接口参数:"></midea-title-bar>
        <midea-button text="获取通讯录" @mideaButtonClicked="mideaButtonClicked">
        </midea-button>
        <midea-title-bar title="代码"></midea-title-bar>
        <text class="display-block">
            nativeService.getAddressBookPerson(messageParam).then(
            (resp) => {
            this.result = resp
            }
            )
...
module.exports = {
    ...
    methods: {
        ...
        mideaButtonClicked() {
            nativeService.getAddressBookPerson(this.messageParam).then(
                (resp) => {
                    this.result = resp
                }
            ).catch((error) => {
