<template>
    <div>
        <div class="item-wrapper" :class="[type=='spaceBetween'?'item-wrapper-sb':'']" v-for="(row, i) in dList" :key="i">
            <div class="item-inner-wrapper" :class="[activeClass(i*cols+index)]" :style="innerStyle" v-for="(item,index) in row" :key="index" @click="itemClicked(i*cols+index)">
                <image :class="[direction=='v'?'item-img-v':'item-img-h']" :src="item.img||''"></image>
                <text v-if="item.title" :class="[direction=='v'?'item-txt-v':'item-txt-h']">{{item.title}}</text>
            </div>
        </div>
    </div>
</template>
